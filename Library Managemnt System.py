...
def checkbook(bno):
    try:
        con = sc.connect(host='localhost', user='root',
                         passwd='12345', database='library')
        cur = con.cursor()
...
def book():
    ...
    ch = 'y'
    while ch == 'y' or ch == 'Y':
        con = sc.connect(host='localhost', database='library',
        ...
        try:
            ...
            cur.execute("insert into book(bno, bname, author, price, copy) values({}, '{}', '{}', {}, {})".format(
...
def member():
    ...
    while ch == 'y' or ch == 'Y':
        ...
        try:
            ...
            cur.execute("insert into member(id, name, join_date, mob) values({}, '{}', '{}', '{}')".format(
                mno, name, jdate, mob))
            con.commit()
            print('************************************************************')
...
def register():
    ...
    while ch == 'y' or ch == 'Y':
        ...
        try:
            ...
            cur.execute("insert into register(is_no, bno, id, issue_date, dew_date) values({}, {}, {}, '{}', '{}')".format(
                ino, bno, mno, idate, dew))
            con.commit()
            cur.execute(
                "update book set copy=copy-1 where bno={}".format(bno))
            con.commit()
            print('************************************************************')
...
def view():
    ...
    while ch == 'y' or ch == 'Y':
        try:
            ...
            cur.execute(
                "Select name, bname, issue_date, dew_date, return_date, mob from book b, register r, member m where b.bno=r.bno and m.id=r.id and m.id={}".format(mno))
            data = cur.fetchone()
            os.system('cls')
            print('************************************************************')
...
def list_all():
    ...
    while ch == 'y' or ch == 'Y':
        try:
            ...
            print('************************************************************')
            cur.execute(
                "Select is_no, name, bname, dew_date from book b, register r, member m whereb.bno=r.bno and m.id=r.id")
            data = cur.fetchall()
            os.system('cls')
            print('************************************************************')
    ...
    while ch == 'y' or ch == 'Y':
        try:
            ...
            if opt == 1:
                ...
                cur.execute(
                    "update register set dew_date='{}' where is_no={}".format(dew, ino))
                con.commit()
                print('************************************************************')
                ...
                while checki_book_member(mno):
                    print(
                        '************************************************************')
                    print("\tBooks pending = ", checki_book_member(
    ...
    while ch == 'y' or ch == 'Y':
        try:
            ...
            while checkissue(ino) == 0:
                ...
                ino = int("103"+str(ino))
            cur.execute(
                "update book set copy=copy+1 where bno in (select bno from register where is_no={})".format(ino))
            con.commit()
